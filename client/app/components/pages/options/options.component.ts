...
export class OptionsComponent implements OnInit, AfterViewInit, OnDestroy  {
  ...
  breakpointObserver$ = this.breakpointObserver.observe('(min-width: 560px)').pipe(map(({matches}) => matches ? 'horizontal' : 'vertical'))
  ...
  ngOnInit() {
    this.genreQuery$.valueChanges.pipe(
      first(({ data: { genres: { edges } }}) => edges !== undefined),
      map(({ data: { genres: { edges } }}) => edges?.filter(({ node: { selected } }) => selected) ?? []),
